...
def format_table(results: BenchmarkResult) -> str:
    ...
    if by_era:
        lines.append("By Era")
        lines.append(
            f"  {'Era':<12} | {'Inputs':>7} | {'K (med)':>10} | {'Core (med)':>10} | {'Ratio':>7}"
        )
        lines.append(f"  {'-' * 12}-+-{'-' * 7}-+-{'-' * 10}-+-{'-' * 10}-+-{'-' * 7}")
        for era, stats in by_era.items():
            ratio_str = f"{stats['ratio']:.0f}x" if stats["ratio"] else "N/A"
            lines.append(
                f"  {era:<12} | {stats['count']:>7,} | "
                f"{_ns_to_human(stats['k_median_ns']):>10} | "
                f"{_ns_to_human(stats['core_median_ns']):>10} | "
                f"{ratio_str:>7}"
            )
        lines.append("")
