...
from pyk.kast.inner import KApply, KRewrite, KSort, KToken, KVariable, bottom_up
from pyk.kast.manip import split_config_and_constraints
from pyk.konvert._kast_to_kore import kast_to_kore
...
def test_llvm_prove(
    ...
    _CELL_DEFAULTS: dict[str, tuple[str, KToken | KApply]] = {
        ...
        "TapleafHashCell": ("<tapleafHash>", KToken('b""', KSort("Bytes"))),
        "TapscriptBytesCell": ("<tapscriptBytes>", KToken('b""', KSort("Bytes"))),
        "CodesepPosCell": ("<codesepPos>", KToken("4294967295", KSort("Int"))),
    }
    ...
    def _concretize(t):
        ...
        if isinstance(t, KVariable):
            sort_name = t.sort.name if t.sort else None
            if sort_name in _CELL_DEFAULTS:
                label, default = _CELL_DEFAULTS[sort_name]
                return KApply(label, [default])
            if sort_name == "Bytes":
                return KToken('b""', KSort("Bytes"))
