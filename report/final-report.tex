...
mainnet-derived benchmark dataset, and a suite of 71 mechanized K proof claims
covering arithmetic opcode correctness, historical consensus bug fixes, and
flag-dependent behavioral boundaries.  On a 288,190-input dataset, the latest
benchmark run on an Apple M4 Pro produces 288,045 paired K/Core measurements
with zero mismatches; K verifies the paired inputs in 175.4 seconds, compared
with 84.2 seconds for \texttt{libbitcoinconsensus}.  We also report the
...
\begin{itemize}
  ...
  configuration.
  \item It presents 71 mechanized proof claims verified by the K Haskell Kore
  prover, covering arithmetic opcode correctness, historical soft-fork bug
  fixes (MINIMALDATA, NULLDUMMY, CLEANSTACK,
...
for symbolic proof of reachability claims.  K-Bitcoin Script includes 71
mechanized proof claims across 19 specification files, plus 2 LLVM-backed
proofs for hash time-locked contracts (HTLCs).
...
initializes the semantics under given consensus flags and scripts.  The Haskell
Kore prover discharges claims by symbolic execution over the rewrite rules, with
arithmetic side conditions delegated to an SMT backend.
...
field.  The 71 total claims span arithmetic, stack manipulation, flow-control
boundaries, cryptographic shape checks, and the historical bug categories
described above.
...
The 71 existing proof claims demonstrate that the K proof infrastructure is
functional on realistic Bitcoin Script specifications.  Future work can extend
this to higher-level properties: for example, that a time-locked contract
...
CHECKSIG input order.  The two existing HTLC proofs are a proof of concept for
this direction.
