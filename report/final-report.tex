...
executable semantics of \bitcoin Script in the \kframework.  Following the
style of prior large K semantics such as KEVM, KJS, and K-Java, we emphasize
the engineering choices needed to make a language definition simultaneously
executable, testable, and useful as a reference artifact.  The semantics covers
legacy, SegWit v0, and Taproot/Tapscript validation paths, including BIP-143,
BIP-341, and BIP-342 signature hashing.  We validate it against Bitcoin Core's
script and transaction test vectors and against a mainnet-derived benchmark
dataset.  On a 288,190-input dataset, the latest benchmark run produces 288,045
paired K/Core measurements with zero mismatches; K verifies the paired inputs
in 175.4 seconds, compared with 84.2 seconds for \texttt{libbitcoinconsensus}.
We also report the project's AI-assisted development methodology: language
model agents were used for implementation and debugging, while Bitcoin Core
vectors, BIP vectors, \texttt{libbitcoinconsensus}, and mainnet replay served
as the ground truth.
\end{abstract}
...
chunking, native cryptographic hooks, and a done-phase cleanup rule that removes
large no-longer-observed cells from final configurations.
...
for particular opcode families.
...
\begin{figure*}[t]
  ...
  configuration and phase machinery live in \texttt{script-semantics.k}; opcode
  families are separated into smaller modules, while cryptographic primitives
  are delegated to native hooks.}
...
start in \texttt{script-semantics.k}.  The main exception is
...
signature, but they do so differently.  The \kcell{scriptCode} cell stores the
current BIP-143 or legacy script code.  The \kcell{pendingCodesepTail} cell is
written by the decoder just before a decoded \texttt{OP\_CODESEPARATOR}; if
the separator is actually executed, the opcode rule can promote that tail into
\kcell{scriptCode}.  Tapscript uses \kcell{tapleafHash},
\kcell{tapscriptBytes}, and \kcell{codesepPos} for the BIP-342 extension
fields.
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \kcell{scriptCode} & Active signature script code & Signature rules \\
...
The configuration also includes a concrete-runtime cleanup rule.  Once a
successful run reaches \texttt{done} and the \kcell{k} cell drains, the LLVM
backend rewrites large byte and list cells such as \kcell{tx}, \kcell{prevouts},
\kcell{witness}, and \kcell{sighash} to empty values.  This rule is tagged
concrete so proof-oriented backends do not see it.  The rule does not change
the result observed by the verifier, which reads only success/failure and the
final error state, but it dramatically reduces the final KORE pattern that the
runtime must serialize after each benchmark input.
...
\begin{figure*}[t]
  ...
  path; the witness branch targets either v0 or v1 execution.  The actual K
  rules include additional side conditions such as push-only checks,
...
truthiness, clean-stack, and unexpected-witness checks.
...
script that is a witness program enters the witness verifier.  This split is
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \texttt{done} & None & Runtime cleanup / observable success \\
    \bottomrule
  \end{tabular}
  }
\end{table}

Failures are explicit.  The K item \texttt{\#fail(ERR)} writes a Bitcoin
Core-style error string into \kcell{error} and clears \kcell{k}.  Negative test
vectors are therefore not merely stuck terms: they terminate with structured
failure labels such as \texttt{SCRIPT\_SIZE}, \texttt{SIG\_PUSHONLY},
\texttt{WITNESS\_PROGRAM\_MISMATCH}, \texttt{CLEANSTACK}, or
\texttt{SCHNORR\_SIG}.  This is valuable for debugging because a mismatch can
be classified as the wrong success value, the wrong error class, or a harness
problem before looking at the full configuration trace.
...
The runtime interface is intentionally thin.  It translates host data into K
cells, starts the semantics, and translates the final success or error state
back to Python.  Consensus behavior is not encoded in Python branches except
for preparing the input material.  This matters for the report's claim: the
artifact being evaluated is the K semantics, not a Python reimplementation
that happens to call K for a subset of opcodes.
...
expected failures, consisting of malformed \texttt{BADTX} cases and legacy
\texttt{CONST\_SCRIPTCODE} sighash edge cases.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrl}
    ...
    Taproot vectors & 26 & 26 & Key path, control blocks, annex \\
    \bottomrule
  \end{tabular}
  }
\end{table}
...
The project includes direct tests for legacy, BIP-143, and BIP-341 sighash
entry points, plus C-hook isolation tests that compare hook output with Python
\texttt{hashlib} implementations.  BIP-342 sighash is currently covered
indirectly through full script verification on mainnet Tapscript inputs rather
than by a standalone published vector file.  This is a useful distinction:
mainnet verification is strong end-to-end evidence, but a direct BIP-342 vector
suite would make the coverage argument cleaner.
...
input to match the prior v10 artifact.

\subsection{Overall Results}

Table~\ref{tab:overall} gives the main result.  K verifies the paired dataset
in 175.4 seconds, while Core takes 84.2 seconds.  The wall-time ratio is
2.1$\times$.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Engine & Inputs & Total & Throughput & Median \\
    \midrule
...
with a median net verification cost of about 26.1 $\mu$s.
...
representative inputs, K takes 105.3 seconds and Core takes 13.3 seconds, for a
7.9$\times$ total-time ratio.  On stress inputs, K takes 70.1 seconds and Core
takes 70.9 seconds, putting the two essentially at parity.  This result is
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrr}
    \toprule
    Category & Inputs & K total & Core total \\
    \midrule
...
The immediate application is an executable reference model for testing
consensus-sensitive behavior.  Because the same K rules execute script tests,
transaction vectors, and mainnet-derived inputs, regressions in opcode
semantics or sighash dispatch can be caught without creating a second model.
The benchmark harness also functions as a performance regression test: it
identified when the v10 binary path was present in the code but bypassed by a
later benchmark runner change.

The next natural application is offline audit.  A K-backed verifier can replay
selected blocks or transactions independently of Bitcoin Core's interpreter and
flag disagreements for investigation.  This is not a node replacement; it is a
high-assurance cross-checking tool.
...
transaction vectors are expected failures, and direct BIP-342 sighash vectors
should be added to complement mainnet Tapscript coverage.  Benchmark artifacts
are large and currently live on the remote benchmark machine rather than in the
repository; they should be published as release artifacts or archived with the
final report.  The performance numbers are single-run measurements.  A final
paper-quality evaluation should include repeated runs, variance, exact hardware
...
measurements and a 2.1$\times$ wall-time ratio against
